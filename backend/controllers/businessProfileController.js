...
export async function createBusinessProfile(req, res) {
  try {
    ...
    return res.status(201).json({
      success: true,
      data: saved,
      message: "Business Profile Created Succesfully.",
    });
  } catch (err) {
...
export async function updateBusinessProfile(req, res) {
  try {
    ...
    return res.status(200).json({
      success: true,
      data: updated,
      message: "Business Profile updated succesfully.",
    });
  } catch (err) {
...
// To get MyBusinessProfile
export async function myBusinessProfile(req, res) {
  try {
    const { userId } = getAuth(req);
    if (!userId) {
      return res
